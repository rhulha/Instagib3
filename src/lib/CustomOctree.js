...
var CustomOctree = ( function () {
	...
	function CustomOctree( box ) {
		this.triangles = [];
		this.box = box;
		this.subTrees = [];
	}
	Object.assign( CustomOctree.prototype, {
			...
			for ( var x = 0; x < 2; x ++ ) {
				for ( var y = 0; y < 2; y ++ ) {
					for ( var z = 0; z < 2; z ++ ) {
						...
						subTrees.push( new CustomOctree( box ) );
					}
				}
			}
		...
		capsuleIntersect: function ( capsule ) {
			...
			if ( hit ) {
				...
				var depth = collisionVector.length();
				return { normal: collisionVector.normalize(), depth: depth, userData: userData };
			}
			return false;
		},
